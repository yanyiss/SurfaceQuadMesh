...
bool check_in_triangle_face(const Matrix3Xd& tri, const Vector3d& p)
{
	...
	Vector3d v1 = tri.col(1) - tri.col(0); Vector3d v2 = tri.col(2) - tri.col(0);
	Vector3d n = v1.cross(v2);
	double face_area = n.norm(); n.normalize(); double all_area = 0;
	for(unsigned i=0; i < tri.cols(); ++i)
	{
		unsigned next_i = ( i+1 )%tri.size(); unsigned prev_i = ( i + tri.size() - 1 )%tri.size();
		v1 = tri.col(next_i) - p; v2 = tri.col(prev_i) - p;
		double area = v1.cross(v2).dot(n); all_area += area;
		if(area < 0)
		{
			return false;
		}
